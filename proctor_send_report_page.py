#send session reports via email
...
class SendReportPage(tk.Frame):
    def __init__(self, parent):
        super().__init__(parent, bg="white")
        self.selected_files = []  #list of attached files

        #page title
        tk.Label(self, text="Send Report via Email", font=("Helvetica", 16, "bold"), bg="white").pack(pady=15)
        ...
        note = tk.Label(
            self,
            text="For issues, please contact admin at fyprojectkey@gmail.com",
            font=("Helvetica", 9),
            bg="white",
            fg="black"
        )
    ...
    def labeled_entry(self, label_text):
        #create label and text entry row
    ...
    def attach_files(self):
        #attach one or more report files to be sent
    ...
    def send_email(self):
        ...
        try:
            ...
            server.starttls()
            server.login(sender_email, "enka rqfz ghzy sjhy")  #use app password
    ...
    def preload_file(self, file_path):
        #preload ZIP report to send from another page
    ...
    def log_email(self, sender, recipient, subject, body, attachment_path):
        #log the send email to SQLite for history
